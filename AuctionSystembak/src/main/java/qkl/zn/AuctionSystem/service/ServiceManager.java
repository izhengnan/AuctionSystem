...
import qkl.zn.AuctionSystem.config.SystemConfig;
...
public class ServiceManager {
  ...
  /**
   * @notice: must use @Qualifier("AuctionSystemService") with @Autowired to get this Bean
   */
  @Bean("AuctionSystemService")
  public Map<String, AuctionSystemService> initAuctionSystemServiceManager() throws Exception {
    Map<String, AuctionSystemService> serviceMap = new ConcurrentHashMap<>(this.hexPrivateKeyList.size());
    for (int i = 0; i < this.hexPrivateKeyList.size(); i++) {
    	...
    	AuctionSystemService auctionSystemService = new AuctionSystemService();
    	auctionSystemService.setAddress(this.config.getContract().getAuctionSystemAddress());
    	auctionSystemService.setClient(this.client);
    	...
    	auctionSystemService.setTxProcessor(txProcessor);
    	serviceMap.put(userAddress, auctionSystemService);
    }
    log.info("++++++++AuctionSystemService map:{}", serviceMap);
    return serviceMap;
  }
}
